...
(define (interp expr env)
  (type-case FAE expr
    ...
    [app (fun body)
         (local([define funV (interp fun env)])
           (if (closureV? funV)
               (let* ([funV-params (closureV-param funV)]
                      [fbody (length funV-params)]
                      [expr (length body)])
                 (interp (closureV-body funV)
                         (if (= fbody expr) (mParam funV-params body env (closureV-env funV))
...
;;This is my failure, and it shall remain here as a token of shame. 
 ...
 ;        (local ([define fun-val (interp fun-id env)])
  ;         (interp (closureV-body fun-val) 
   ;                (aSub (closureV-param fun-val)
    ...
    ;                     (mParam )  ;;this is my supposed solution, it applies a map on the list that is receiving, but it says later on that it revieces a '(x) (solved with cparse)
